...
void send_respond_head(int cfd, int no, const char* desp, const char* type, long len)
{
    ...
    sprintf(buf+strlen(buf), "Content-LengthL:%ld\r\n", len);
    send(cfd, buf, strlen(buf), 0);
    //空行
    send(cfd, "\r\n", 2, 0);
}
...
//编码。用作会写浏览器的时候，将除字母数字及/._~意外的字符转义后回写
...
const char* get_file_type(const char* name)
{
    ...
    dot = strrchr(name, '.');
    if (dot == NULL) return "text/plain; charest=utf-8";
    ...
    if (strcmp(dot, ".mpv") || strcmp(dot, ".qt") == 0)
